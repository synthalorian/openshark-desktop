<script>
  ...
  onMount(async () => {
    try {
      output = await runCommand(['models']);
    } catch (e) {
      output = `Error: ${e}`;
    } finally {
      loading = false;
...
<div class="view">
  <header>
    <h1 class="glow-text">◆ Models</h1>
    <p class="sub">Universal — any model, any provider, local or cloud.</p>
  </header>
  <pre class="output card">{loading ? 'Loading models…' : output || 'No models configured.'}</pre>
</div>
